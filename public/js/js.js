...
$(function() {
        ...
        $.ajax({
            ...
            if (response.Status == 'Ok') {
                setTimeout(function() {
                    ...
                    var data = JSON.parse(JSON.stringify(response.Data));
                    //*********************** Cayro **************************
                    buscaImagenes(data.expense.id);
                    // Sincronizado desde el teléfono.

                    if (data.expense.sincronizado == '1') {
                        $("#imgSincronizado").attr("src", server + 'img/telefono.png');
                        $("#titleSincronizado").attr("title", 'Registrado desde teléfono movil.');
                    } else {
                        $("#imgSincronizado").attr("src", server + 'img/computer.png');
                        $("#titleSincronizado").attr("title", 'Registrado desde la aplicación web.');
                    }

                    var tipoUser = $("#tipoUser").val();
                    fotoUser = server + 'img/usuario.svg';
                    VB = data.expense.vb_sup_bit == '1' ? 'VB Aprobado' : 'VB Rechazado';
                    tileSup = 'Visto Bueno Supervisor.<br> <img alt="Usuario" width="100" height="100" src="' + fotoUser + '" ><br><br>' + data.expense.user_vb_sup + '<br>' + data.expense.fec_vb_sup + '<br>Estado:<strong style="color: limeGreen;">' + VB + '</strong><br>' + data.expense.coment_vb_sup;

                    VBDoc = data.expense.vb_doc_bit == '1' ? 'VB Aprobado' : 'VB Rechazado';
                    titleVbDoc = 'Visto Bueno Documento.<br> <img alt="Usuario" width="100" height="100" src="' + fotoUser + '" ><br><br>' + data.expense.user_vb_doc + '<br>' + data.expense.fec_vb_doc + '<br>Estado:<strong style="color: limeGreen;"> ' + VBDoc + '</strong><br>' + data.expense.coment_vb_doc;

                    VBDocRec = data.expense.doc_rec_bit == '1' ? 'Doc. recibido.' : '';
                    titleDocRec = 'Recepción de Documentos.<br> <img alt="Usuario" width="125" height="125" src="' + fotoUser + '" ><br><br>' + data.expense.user_doc_rec + '<br>' + data.expense.fec_doc_rec + '<br>' + 'Estado: <strong style="color:limeGreen;"> ' + VBDocRec;

                    $("#titleVbSup").attr("data-original-title", tileSup);
                    $("#titleVbDoc").attr("data-original-title", titleVbDoc);
                    $("#titleDocRec").attr("data-original-title", titleDocRec);

                    if (tipoUser == 'R') {
                        if (data.expense.vb_sup_bit == '1') {
                            $("#divVbSup").html('<i class="text-success fa-2x fas fa-check"></i>');
                        } else if (data.expense.vb_sup_bit == '2') {
                            $("#divVbSup").html('<i class="text-danger fa-2x fas fa-ban"></i>');
                        } else {}

                        if (data.expense.vb_doc_bit == '1') {
                            $("#divVbDoc").html('<i class="text-success fa-2x fas fa-check"></i>');
                        } else if (data.expense.vb_doc_bit == '2') {
                            $("#divVbDoc").html('<i class="text-danger fa-2x fas fa-ban"></i>');
                        } else {}

                        if (data.expense.doc_rec_bit == '1') {
                            $("#divVbDocRec").html('<i class="text-success fa-2x fas fa-check"></i>');
                        } else {
                            //$("#divVbDocRec").html('<i class="text-danger fa-2x fas fa-ban"></i>');
                        }

                    } else {
                        // Visto bueno Supervisador.
                        reiniciaTooltipSup();
                        if (data.expense.vb_sup_bit == '1') {
                            $("#idVbSup").prop("checked", true);
                            $("#titleVbSup").attr("data-original-title", tileSup);
                            $("#checkbox1").attr("style", "box-shadow:0 0 0 2px limeGreen,0 0 2px limeGreen;");
                        } else if (data.expense.vb_sup_bit == '2') {
                            $("#idVbSup").prop("checked", false);
                            $("#titleVbSup").attr("data-original-title", tileSup);
                            $("#checkbox1").attr("style", "box-shadow:0 0 0 2px FireBrick,0 0 2px FireBrick;");
                        } else {
                            $("#idVbSup").prop("checked", false);
                            $("#titleVbSup").attr("title", '');
                        }

                        // Visto bueno Documento.
                        reiniciaTooltipDoc();
                        if (data.expense.vb_doc_bit == '1') {
                            $("#idVbDoc").prop("checked", true);
                            $("#titleVbDoc").attr("data-original-title", titleVbDoc);
                            $("#checkbox2").attr("style", "box-shadow:0 0 0 2px limeGreen,0 0 2px limeGreen;");
                        } else if (data.expense.vb_doc_bit == '2') {
                            $("#idVbDoc").prop("checked", false);
                            $("#titleVbDoc").attr("data-original-title", titleVbDoc);
                            $("#checkbox2").attr("style", "box-shadow:0 0 0 2px FireBrick,0 0 2px FireBrick;");
                        } else {
                            $("#idVbDoc").prop("checked", false);
                            $("#titleVbDoc").attr("title", '');
                        }

                        // Documento recibido.
                        reiniciaTooltipDocRec();
                        if (data.expense.doc_rec_bit == '1') {
                            $("#idDocRec").prop("checked", true);
                            fotoUser = server + 'img/usuario.svg';
                            $("#titleDocRec").attr("data-original-title", titleDocRec);
                        } else {
                            $("#idDocRec").prop("checked", false);
                            $("#titleDocRec").attr("title", '');
                        }
                    }

                    $("#titleVbSup").attr("data-placement", "left");
                    $("#titleVbDoc").attr("data-placement", "right");
                    $("#titleDocRec").attr("data-placement", "right");

                    $('#titleSincronizado').tooltip();
                    $('#titleVbSup').tooltip();
                    $('#titleVbDoc').tooltip();
                    $('#titleDocRec').tooltip();
                    //********************************************************
                    ...
                    $("#ruc").val(data.expense.ruc).attr("disabled", true);
                    $("#ruc-hide").val(data.expense.ruc);
                    reiniciaTooltipRazonSocial();
                    razonSocial = data.expense.razon.substr(0, 20);
                    $("#razon-val").attr("data-original-title", data.expense.razon);
                    $('#razon-val').tooltip();
                    $("#razon").text(razonSocial).css("color", "#5c5c5c");
                    $("#razon").attr("data-edit", 1);
                    $("#number-prefix").val(data.expense.num_prefijo).attr("disabled", true);
                    $("#number-serie").val(data.expense.num_serie).attr("disabled", true);
                    if ($("#regimen").length == 1) {
    ...
    // Actualiza Visto Bueno Supervisor
    $("#idVbSup").on("click", function(e) {

        var tipoUsuario = $("#tipoUser").val();
        if (tipoUsuario != "S") {
            // alertify.defaults.theme.ok = "btn btn-primary";
            // alertify.alert("Opción deshabilitada..!",'<h4 style="color=red;">El perfil de su <strong>Usuario</strong> no tiene permitido cambiar el status de este gasto.</h4>', function(){});
            alertify.set('notifier', 'position', 'top-center');
            alertify.error('El perfil de su <strong>Usuario</strong> no tiene permitido cambiar el status de este gasto.<br><strong>Permitido solo al Supervisor.</strong>');
            return false;
        }

        appDire = server.split('/');
        appDire = appDire[1];
        $(document).off('focusin.modal');
        var estaSeleccionado = $('#idVbSup').is(":checked");
        if (estaSeleccionado) {
            $("#idVbSup").prop("checked", false);
        } else {
            $("#idVbSup").prop("checked", true);
        ...
        idGasto = $("#idgasto").val();
        var fecha = new Date();
        fecha_hoy = fecha.getDate() + "/" + (fecha.getMonth() + 1) + "/" + fecha.getFullYear() + '  ' + fecha.getHours() + ":" + fecha.getMinutes() + ":" + fecha.getSeconds();

        swal({
            title: 'Visto Bueno Supervisor.',
            html: '<br><label style="float: left;">Comentario:</label><textarea id="comentarioSup" name="comentarioSup" class="form-control" rows="6" required maxlength="100"></textarea>',
            inputPlaceholder: 'Comentario del porque aprueba o rechaza el visto bueno...',
            showCancelButton: true,
            confirmButtonText: 'Aprobar VB',
            cancelButtonText: 'Rechazar VB',
        }).then((result) => {
            comentarioSup = $("#comentarioSup").val();
            if (result.dismiss === Swal.DismissReason.confirm) {
                ...
                $("#idVbSup").prop("checked", true);
                valorVB = 1;
                colorEstado = 'limeGreen;';
                vbStatus = 'VB Aprobado';
                $("#checkbox1").attr("style", "box-shadow:0 0 0 2px limeGreen,0 0 2px limeGreen;");
                cambiaStatusVB(idGasto, valorVB, 'vistoBueno-Supervisor', comentarioSup, appDire);
            } else if (result.dismiss === Swal.DismissReason.cancel) {

                Swal(
                    'Rechazado!',
                    'Visto Bueno Rechazado por el Supervisor :(',
                    'error'
                )
                $("#idVbSup").prop("checked", false);
                valorVB = 2;
                colorEstado = 'FireBrick;';
                vbStatus = 'VB Rechazado';
                $("#checkbox1").attr("style", "box-shadow:0 0 0 2px FireBrick,0 0 2px FireBrick;");
                cambiaStatusVB(idGasto, valorVB, 'vistoBueno-Supervisor', comentarioSup, appDire)
            }
            ...
            fotoUser = server + 'img/usuario.svg';
            usuario_sup = "";
            tileSup = 'Visto Bueno Supervisor.<br> <img alt="Usuario" width="100" height="100" src="' + fotoUser + '" ><br><br>' + usuario_sup + '<br>' + fecha_hoy + '<br>Estado:<strong style="color: ' + colorEstado + '"> ' + vbStatus + '</strong><br>' + comentarioSup;
            $("#titleVbSup").attr("data-original-title", tileSup);
    ...
    function cambiaStatusVB(idGasto, valorVB, valorController, comentario, appDire) {

        $.ajax({
            url: server + valorController,
            type: "post",
            dataType: "html",
            data: {
                'id': idGasto,
            ...
            success: function(response) {
                alertify.success('Status cambiado sastifactoriamente.');
            },
            error: function() {
                responseUI("No se puede acceder al servidor", "red");
                $(".message-expense").text('No se pueden recuperar los datos del servidor.').show();
    ...
    // Actualiza Visto Bueno Documento Contabilidad
    $("#idVbDoc").on("click", function(e) {
        var tipoUsuario = $("#tipoUser").val();
        if (tipoUsuario != "C") {
            alertify.set('notifier', 'position', 'top-center');
            alertify.error('El perfil de su <strong>Usuario</strong> no tiene permitido cambiar el status de Visto Bueno al Documento.<br><strong>Permitido solo el área contable.</strong>');
            return false;
        }
        appDire = server.split('/');
        appDire = appDire[1];
        $(document).off('focusin.modal');
        var estaSeleccionado = $('#idVbDoc').is(":checked");
        if (estaSeleccionado) {
            $("#idVbDoc").prop("checked", false);
        } else {
            $("#idVbDoc").prop("checked", true);
        ...
        swal({
            title: 'Visto Bueno Documento.',
            html: '<br><label style="float: left;">Comentario:</label><textarea id="comentarioDoc" name="comentarioDoc" class="form-control" rows="6" required maxlength="100"></textarea>',
            inputPlaceholder: 'Comentario del porque aprueba o rechaza el visto bueno del Documento...',
            showCancelButton: true,
            confirmButtonText: 'Aprobar VB',
            cancelButtonText: 'Rechazar VB',

        }).then((result) => {
            comentarioDoc = $("#comentarioDoc").val();
            if (result.dismiss === Swal.DismissReason.confirm) {
                Swal(
                    'Aprobado!',
                ...
                vbStatus = 'VB Aprobado';
                $("#checkbox2").attr("style", "box-shadow:0 0 0 2px limeGreen,0 0 2px limeGreen;");
                cambiaStatusVB(idGasto, valorVB, 'vistoBueno-Documento', comentarioDoc, appDire);
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                Swal(
                    'Rechazado!',
                    'Visto Bueno Rechazado por el Documento :(',
                    'error'
                )
                $("#idVbDoc").prop("checked", false);
                valorVB = 2;
            ...
            fotoUser = server + 'img/usuario.svg';
            usuario_sup = "";
            tileSup = 'Visto Bueno Documento.<br> <img alt="Usuario" width="100" height="100" src="' + fotoUser + '" ><br><br>' + usuario_sup + '<br>' + fecha_hoy + '<br>Estado:<strong style="color: ' + colorEstado + '"> ' + vbStatus + '</strong><br>' + comentarioDoc;
            $("#titleVbDoc").attr("data-original-title", tileSup);
        })
    ...
    // Documento recibido.
    $("#idDocRec").on("click", function(e) {
        var tipoUsuario = $("#tipoUser").val();
        if (tipoUsuario != "C") {
            alertify.set('notifier', 'position', 'top-center');
            alertify.error('El perfil de su <strong>Usuario</strong> no tiene permitido cambiar el status de Documentos Recibidos.<br><strong>Permitido solo el área contable.</strong>');
            return false;
        }
        var valorVB = $('input:checkbox[name=idDocRec]:checked').val();
        if (valorVB == 'on') {
            alertify.success('Documento recibido.')
            valorVB = '1';
        } else {
            alertify.error('Documento no recibido.');
            valorVB = '0';
        }
        idGasto = $("#idgasto").val();
        $.ajax({
            url: server + 'documento-Recibido',
            type: "post",
            dataType: "html",
            data: {
                'id': idGasto,
    ...
    function reiniciaTooltipSup() {
        $("#titleVbSup").removeAttr('title');
        $("#titleVbSup").removeAttr('data-original-title');
        $("#titleVbSup").removeAttr("data-placement");
    }

    function reiniciaTooltipDoc() {
        $("#titleVbDoc").removeAttr('title');
        $("#titleVbDoc").removeAttr('data-original-title');
        $("#titleVbDoc").removeAttr("data-placement");
    }

    function reiniciaTooltipDocRec() {
        $("#titleDocRec").removeAttr('title');
        $("#titleDocRec").removeAttr('data-original-title');
        $("#titleDocRec").removeAttr("data-placement");
    }

    function reiniciaTooltipRazonSocial() {
        $("#razonSocialFull").removeAttr('title');
        $("#razonSocialFull").removeAttr('data-original-title');
        $("#razonSocialFull").removeAttr("data-placement");
    }

    function buscaImagenes(idGasto) {

        $.ajax({
            url: server + 'buscar-imagenes',
            type: "post",
            dataType: "html",
            data: {
                id: idGasto,
            ...
            success: function(response) {
                ...
                $("#esperaImagenes").hide();
                if (response == '""') {
                    $("#imagenPrincipal").attr("src", server + "img/sin-imagen.jpg");
                    $("#imagenPrincipal").show();
                    $("#divImagenPrincipal").show();
                }
                $.each(json, function(key, value) {
                    imagenAux = value.DIRECTORY + value.ID + '.' + value.EXTENSION;
                    idd = i.toString();

                    if (i == 1) {
                        $("#imagenPrincipal").attr("src", server + imagenAux);
                        $("#imagenPrincipal").attr("data-zoom-image", server + imagenAux);
                        $("#imagenPrincipal").elevateZoom({
                            zoomWindowPosition: 10,
                            zoomWindowWidth: 600,
                            zoomWindowHeight: 575,
                            tint: true,
                            tintColour: '#bc3c9f',
                            tintOpacity: 0.5,
                            scrollZoom: true,
                    ...
                    $("#imagen" + idd).attr("src", server + imagenAux);
                    $("#divImagen" + idd).show();
                    i += 1;
                });
            },
    ...
    $("#imagen1").on("click", function(e) {
        reseteaZoom();
        imagen = $("#imagen1").attr('src');
        $("#imagenPrincipal").attr("src", imagen);
        $("#imagenPrincipal").attr("data-zoom-image", imagen);
        $("#imagenPrincipal").elevateZoom({
            zoomWindowPosition: 10,
            zoomWindowWidth: 600,
            zoomWindowHeight: 575,
    ...
    $("#imagen2").on("click", function(e) {
        reseteaZoom();
        imagen = $("#imagen2").attr('src');
        $("#imagenPrincipal").attr("src", imagen);
        $("#imagenPrincipal").attr("data-zoom-image", imagen);
        $("#imagenPrincipal").elevateZoom({
            zoomWindowPosition: 10,
    ...
    $("#imagen3").on("click", function(e) {
        reseteaZoom();
        imagen = $("#imagen3").attr('src');
        $("#imagenPrincipal").attr("src", imagen);
        $("#imagenPrincipal").attr("data-zoom-image", imagen);
        $("#imagenPrincipal").elevateZoom({
            zoomWindowPosition: 10,
    ...
    $("#imagen4").on("click", function(e) {
        reseteaZoom();
        imagen = $("#imagen4").attr('src');
        $("#imagenPrincipal").attr("src", imagen);
        $("#imagenPrincipal").attr("data-zoom-image", imagen);
        $("#imagenPrincipal").elevateZoom({
            zoomWindowPosition: 10,
    ...
    $('#expense-register').on('hide.bs.modal', function() {
        reseteaZoom();
    ...
    $('#open-expense-register').click(function(e) {
        $("#imgSincronizado").attr("src", server + 'img/computer.png');
        $("#imagenPrincipal").attr("src", server + "img/sin-imagen.jpg");
        $("#esperaImagenes").hide();
        $("#divImagenPrincipal").show();
        ...
        $("#divVbDoc").html('');
        $("#divVbDocRec").html('');
        $("#titleVbSup").attr("data-original-title", '');
        $("#titleVbDoc").attr("data-original-title", '');
        $("#titleDocRec").attr("data-original-title", '');
    });
    ...
    $("#save-expense").click(function(e) {
        ...
        if (error !== 0) {
            ...
            alertify.confirm('Rendición de gastos.', '<h4 class="text-info">Esta seguro de guardar esta información.?</h4>', function() {
                ...
                alertify.error('Se Cancelo el Proceso para actualizar el gasto.');
            }).set('labels', {
                ok: 'Confirmar',
                cancel: 'Cancelar'
            }).set({
    ...
    $(".search-ruc").on("click", function() {
        ...
        if (ruc.length === 0) {
            ...
            $('#razon').css({
                color: "#5c5c5c"
            }).html('No ha ingresado el RUC.').parent().parent().addClass('has-error').removeClass('has-success');
