...
class GerPPTOProcedure
{
    ...
    public function upload( $input , $year , $category , $user_id )
    {
        ...
        {
            $pdo = DB::getPdo();
            $stmt = $pdo->prepare( 'BEGIN PK_PPTO_GERENTE.SP_CARGA_PPTO_GERENTE( ' . $input . ' , :año , :categoria , :user_id , :rpta , :desc , :list ); end;' );
            $stmt->bindParam( ':año' , $year , PDO::PARAM_INT );
            $stmt->bindParam( ':categoria' , $category , PDO::PARAM_INT );
    ...
    public function update( $ppto_id , $roundAmount , $user_id )
    {
        ...
        {
            $pdo = DB::getPdo();
            $stmt = $pdo->prepare( 'BEGIN PK_PPTO_GERENTE.SP_UPDATE_PPTO_GERENTE( :ppto_id , :monto , :user_id , :rpta , :desc , :data ); end;' );
